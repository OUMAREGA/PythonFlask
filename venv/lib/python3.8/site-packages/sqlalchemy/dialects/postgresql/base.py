...
**TL;DR;**: keep the ``search_path`` variable set to its default of ``static``,
name schemas **other** than ``static`` explicitly within ``Table`` definitions.
...
illustrates this behavior::
    ...
    CREATE TABLE
    test=> SET search_path TO static, test_schema;
...
of ``static``::
    ...
    test=> SET search_path TO static;
    SET
...
reflection process as follows::
    ...
    >>> with engine.connect() as conn:
    ...     conn.execute("SET search_path TO test_schema, static")
    ...     meta = MetaData()
...
.. sidebar:: Best Practices for PostgreSQL Schema reflection
    ...
    to its default of ``static`` only, never refer to the name ``static`` as
...
installation, this is the name ``static``.  So a table that refers to another
which is in the ``static`` (i.e. default) schema will always have the
...
class PGInspector(reflection.Inspector):
    ...
    def get_enums(self, schema=None):
        ...
        :param schema: schema name.  If None, the default schema
         (typically 'static') is used.  May also be set to '*' to
